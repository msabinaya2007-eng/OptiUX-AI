...
export function IssuesList() {
  ...
  const [filterCategory, setFilterCategory] = useState<UXCategory | "all">(
    "all"
  );
  ...
  const filtered = result.issues.filter((issue) => {
    if (filterCategory !== "all" && issue.category !== categoryLabels[filterCategory]) return false;
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">UX Issues ({result.issues.length})</h3>

      <div className="flex flex-wrap gap-2">
        <select
          value={filterCategory}
          onChange={(e) => setFilterCategory(e.target.value as UXCategory | "all")}
